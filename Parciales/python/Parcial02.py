...
def acomodar(lista: list[str]) -> list[str]:
    ...
    for partido in lista:
        if partido == "UP":
            listaFinal.append(partido)
    
    for partido1 in lista:
        if partido1 == "LLA":
            listaFinal.append(partido1)
...
print(pos_umbral(s0, 5)) # 3
print(pos_umbral(s1, 15)) # 4
print(pos_umbral(s2, 9)) # 0
print(pos_umbral(s3, 4)) # -1 """
...
def columnas(lista: list[int]) -> bool:
    primeraMitad = []
    segundaMitad = []
...
def columnas_repetidas(mat: list[list[int]]) -> bool:
    ...
    for i in range(len(mat)):
        if not columnas(mat[i]):
            res = False
    
    return res

m = [[1,2,1,2],[-5,6,-5,6],[0,1,0,1]]
m1 = [[1,2,1,2], [3,4,5,4],[0,1,0,1]]

print(columnas_repetidas(m)) #True
print(columnas_repetidas(m1)) #False
...
naciones= ["arg", "aus", "nz", "sud"]
torneos= {2023:["nz", "sud", "arg", "aus"], 2022:["nz", "sud", "aus", "arg"]}
torneo2= {2023:["nz", "arg", "sud", "fra", "ing"], 2022:["fra", "nz", "arg", "sud", "ing"], 2021: ["nz", "arg", "sud", "ing", "fra"]}
naciones2= ["arg", "ing", "nz", "sud","fra"]
print(cuenta_posiciones_por_nacion(naciones, torneos)) 
print()
print(cuenta_posiciones_por_nacion(naciones2, torneo2))